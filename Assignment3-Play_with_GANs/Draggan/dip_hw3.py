...
with gr.Blocks() as app:
    ...
    def get_face_landmarks(image):
        ...
        if landmarks is None or len(landmarks) == 0:
            return None
        return landmarks[0]  # 取第一个检测到的人脸

    def get_face_slim_points(landmarks):
        """根据关键点生成瘦脸控制点
        Args:
            landmarks: (68, 2) 关键点坐标
        Returns:
            points: dict, 包含start和target点对
        """
        points = {}
        # 左脸颊点
        left_face = landmarks[2:8]
        # 右脸颊点  
        right_face = landmarks[14:10:-1]
        
        # 为每个脸颊点生成向内移动的控制点对
        idx = 0
        for i, point in enumerate(left_face):
            x, y = point
            points[idx] = {
                'target': [int(x), int(y)],
                'start': [int(x-20), int(y)]  # 向内移动10个像素
            }
            idx += 1
            
        for i, point in enumerate(right_face):
            x, y = point
            points[idx] = {
                'target': [int(x), int(y)], 
                'start': [int(x+20), int(y)]  # 向内移动10个像素
    ...
    def on_click_face_slim(global_state):
        ...
        # 生成瘦脸控制点
        points = get_face_slim_points(landmarks)
    ...
    def get_eye_enlarge_points(landmarks, factor=1.3):
        """根据关键点生成大眼控制点
        Args:
            landmarks: (68, 2) 关键点坐标
            factor: 放大比例
        ...
        for eye in [left_eye, right_eye]:
            ...
            for point in eye:
                ...
                points[idx] = {
                    'start': [int(x), int(y)],
                    'target': [int(center_x + (x - center_x) * factor), int(center_y + (y - center_y) * factor)]
                }
                idx += 1
                
        return points

    def get_smile_points(landmarks, offset=10):
        """根据关键点生成微笑控制点
        Args:
            landmarks: (68, 2) 关键点坐标
            offset: 控制嘴角上扬的像素值
        ...
        # 为嘴角生成上扬控制点对
        points[0] = {
            'start': [int(left_mouth[0]), int(left_mouth[1])],
            'target': [int(left_mouth[0]), int(left_mouth[1] - offset)]
        }
        
        points[1] = {
            'start': [int(right_mouth[0]), int(right_mouth[1])],
            'target': [int(right_mouth[0]), int(right_mouth[1] - offset)]
        }
        
        return points

    def on_click_eye_enlarge(global_state):
        """一键大眼功能"""
        image = global_state['images']['image_raw']
        landmarks = get_face_landmarks(image)
        ...
        points = get_eye_enlarge_points(landmarks)
        global_state['points'] = points
    ...
    def on_click_smile(global_state):
        ...
        points = get_smile_points(landmarks)
        global_state['points'] = points
