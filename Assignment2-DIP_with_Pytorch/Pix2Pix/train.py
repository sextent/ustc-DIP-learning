...
def main():
    ...
    for epoch in range(num_epochs):
        ...
        #将这个随机数做os.makedirs的开头，这样可以保证每个文件夹都是唯一的
        # Save model checkpoint every 20 epochs
        if (epoch + 1) % 20 == 0:
            os.makedirs(f'checkpoints/{random_number}', exist_ok=True)
            torch.save(model.state_dict(), f'checkpoints/{random_number}/pix2pix_model_epoch_{epoch + 1}.pth')
